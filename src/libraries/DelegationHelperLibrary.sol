...
import {TownsquareTimeLibrary} from "src/libraries/TownsquareTimeLibrary.sol";
...
library DelegationHelperLibrary {
    ...
    function getPastCheckpointIndex(
        ...
        if (
            ve.checkpoints(mTokenId, nCheckpoints - 1).fromTimestamp <=
            timepoint
    ...
    function earned(
        ...
        uint256 _currTs = TownsquareTimeLibrary.epochStart(
            lmr.lastEarn(_rewardToken, tokenId)
        ...
        _currTs = Math.max(
            ...
            TownsquareTimeLibrary.epochStart(
                lmr.userRewardPointHistory(tokenId, _index).ts
            )
        );

        // get epochs between end of the current epoch and first checkpoint in same epoch as last claim
        uint256 numEpochs = (TownsquareTimeLibrary.epochNext(timepoint) -
            _currTs) / DURATION;
        ...
        if (numEpochs > 0) {
            for (uint256 i = 0; i < numEpochs; i++) {
                ...
                reward +=
                    (balance *
                        lmr.tokenRewardsPerEpoch(_rewardToken, _currTs)) /
                    _supply;
