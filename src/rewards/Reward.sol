...
abstract contract Reward is IReward, Context, ReentrancyGuardTransient {
    ...
    mapping(uint256 => IReward.LockExpiryAndBiasCorrection)
        internal _lockExpiryAndBiasCorrection;
    ...
    mapping(uint256 => IVotingEscrow.GlobalPoint)
        internal _globalRewardPointHistory; // epoch -> unsigned global point
    ...
    /// @inheritdoc IReward
    function lockExpiryAndBiasCorrection(
        uint256 tokenId
    ) external view returns (IReward.LockExpiryAndBiasCorrection memory) {
        return _lockExpiryAndBiasCorrection[tokenId];
    }

    /// @inheritdoc IReward
    function globalRewardPointHistory(
        uint256 _epoch
    ) external view returns (IVotingEscrow.GlobalPoint memory) {
        return _globalRewardPointHistory[_epoch];
    }

    /// @inheritdoc IReward
    function userRewardPointHistory(
        uint256 tokenId,
        uint256 _userRewardEpoch
    ) external view returns (IVotingEscrow.UserPoint memory) {
    ...
    function getPriorSupplyIndex(
        ...
        return
            BalanceLogicLibrary.getPastGlobalPointIndex({
                _epoch: epoch,
    ...
    function getPriorBalanceIndex(
        ...
        return
            BalanceLogicLibrary.getPastUserPointIndex({
    ...
    function earned(
        ...
        uint256 reward = 0;
        uint256 _supply = 1;
        uint256 _currTs = TownsquareTimeLibrary.epochStart({
            timestamp: lastEarn[token][tokenId]
        }); // take epoch last claimed in as starting point
        uint256 _index = getPriorBalanceIndex({
            tokenId: tokenId,
            timestamp: _currTs
        });
        ...
        _currTs = Math.max({
            a: _currTs,
            b: TownsquareTimeLibrary.epochStart({
                timestamp: _userRewardPointHistory[tokenId][_index].ts
            })
        });

        // get epochs between current epoch and first checkpoint in same epoch as last claim
        uint256 numEpochs = (TownsquareTimeLibrary.epochStart({
        ...
        if (numEpochs > 0) {
            for (uint256 i = 0; i < numEpochs; i++) {
                balance = balanceOfNFTAt({
                    tokenId: tokenId,
                    timestamp: _currTs + DURATION - 1
                });
                _supply = Math.max({
                    a: supplyAt({timestamp: _currTs + DURATION - 1}),
                    b: 1
                });
                reward +=
                    (balance * tokenRewardsPerEpoch[token][_currTs]) /
                    _supply;
    ...
    function _deposit(uint256 amount, uint256 tokenId) external {
        ...
        IVotingEscrow.LockedBalance memory locked = IVotingEscrow(ve).locked({
            _tokenId: tokenId
        });

        IVotingEscrow.UserPoint memory userRewardPoint = IVotingEscrow
            .UserPoint({bias: 0, slope: 0, permanent: 0, ts: block.timestamp});

        uint256 userEpoch = userRewardEpoch[tokenId];
        IVotingEscrow.UserPoint
            memory prevUserRewardPoint = _userRewardPointHistory[tokenId][
                userEpoch
            ];
        ...
        if (
            ...
            userRewardPoint.slope = (newBias /
                (newLockEnd - block.timestamp).toInt128());

            _biasCorrection = _getBiasCorrection({
                tokenId: tokenId,
                amount: amount,
                rewardPointSlope: userRewardPoint.slope
            });

            _lockExpiryAndBiasCorrection[tokenId]
                .biasCorrection = _biasCorrection.toInt128();
        }
    ...
    function _withdraw(uint256 amount, uint256 tokenId) external {
        ...
        if (prevLockEnd < block.timestamp)
            prevUserRewardPoint.ts = type(uint256).max;
        ...
        if (prevLockEnd > block.timestamp) {
            ...
            biasCorrections[prevLockEnd] += _lockExpiryAndBiasCorrection[
                tokenId
            ].biasCorrection;
        }
    ...
    function _getReward(
        ...
        for (uint256 i = 0; i < _length; i++) {
            ...
            if (_reward > 0)
                IERC20(tokens[i]).safeTransfer({to: recipient, value: _reward});
    ...
    function _notifyRewardAmount(
        ...
        uint256 epochStart = TownsquareTimeLibrary.epochStart({
            timestamp: block.timestamp
        });
    ...
    function _saveUserRewardPoint(
        ...
        if (
            userEpoch != 0 &&
            TownsquareTimeLibrary.epochStart({
                timestamp: prevUserRewardPoint.ts
            }) ==
            TownsquareTimeLibrary.epochStart({timestamp: block.timestamp})
        ) {
    ...
    function _createGlobalRewardPoints(
        ...
        if (userRewardPoint.permanent != 0 || userRewardPoint.bias != 0) {
            ...
            if (block.timestamp > prevUserRewardPoint.ts) {
                int128 decay = prevUserRewardPoint.slope *
                    (block.timestamp - prevUserRewardPoint.ts).toInt128();
        ...
        if (
            _epoch != 1 &&
            TownsquareTimeLibrary.epochStart({
                timestamp: _globalRewardPointHistory[_epoch].ts
            }) ==
            TownsquareTimeLibrary.epochStart({timestamp: block.timestamp})
        ) {
    ...
    function _getBiasCorrection(
        uint256 tokenId,
        uint256 amount,
        int128 rewardPointSlope
    ) internal view returns (uint256 _biasCorrection) {
        uint256 totalWeight = IVotingEscrow(ve).balanceOfNFT({
            _tokenId: tokenId
        });

        if (totalWeight == amount) return 0;

        int128 mainSlope = IVotingEscrow(ve)
            .userPointHistory({
                _tokenId: tokenId,
        ...
        uint256 dT = TownsquareTimeLibrary.epochNext({
            timestamp: block.timestamp
        }) -
            block.timestamp -
            1;
        ...
        uint256 epochEndRewardBias = amount -
            (rewardPointSlope.toUint256() * dT);
