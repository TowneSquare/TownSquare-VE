...
contract Voter is IVoter, ERC2771Context, ReentrancyGuardTransient {
    ...
    function epochVoteStart(
        ...
        return TownsquareTimeLibrary.epochVoteStart(_timestamp);
    }

    function epochVoteEnd(uint256 _timestamp) external pure returns (uint256) {
        return TownsquareTimeLibrary.epochVoteEnd(_timestamp);
    }
    ...
    function _reset(uint256 _tokenId) internal {
        ...
        for (uint256 i = 0; i < _poolVoteCnt; i++) {
            ...
            if (_votes != 0) {
                ...
                IReward(gaugeToIncentive[gauges[_pool]])._withdraw(
                    _votes,
                    _tokenId
                );
                _totalWeight += _votes;
    ...
    function _vote(
        ...
        for (uint256 i = 0; i < _poolCnt; i++) {
            ...
            if (isGauge[_gauge]) {
                ...
                IReward(gaugeToIncentive[_gauge])._deposit(
                    _poolWeight,
                    _tokenId
                );
    ...
    function vote(
        ...
        if (
            (_timestamp > TownsquareTimeLibrary.epochVoteEnd(_timestamp)) &&
